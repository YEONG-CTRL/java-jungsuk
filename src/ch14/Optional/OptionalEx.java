...
public class OptionalEx {
    public static void main(String[] args) {
        ...
        int result2 = Optional.of("")
                .filter(x -> x.length() >0)
                .map(Integer::parseInt).orElse(-1); // get 사용시 NoSuchElementException

        System.out.println("result1 = " + result1);
        System.out.println("result2 = " + result2);

        Optional.of("456").map(Integer::parseInt)
                .ifPresent(x -> System.out.printf("result 3 = %d%n",x));
//
...
//        System.out.println(optInt2.getAsInt());  // -> No such element Exception
        ...
        Optional<String> opt = Optional.ofNullable(null); // null저장(비어있는것과 동일)
        Optional<String> opt2 = Optional.empty(); // 빈객체 생성
        System.out.println("opt = " + opt);
        System.out.println("opt2 = " + opt2);
        System.out.println("opt.equals(opt) ? " + opt.equals(opt2)); // true
